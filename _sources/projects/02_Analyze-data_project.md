...
I only want theuniquembol == GOOGL` data, so I use a Pandas call. I also want to remove the big drop in price after Mar, 2014, so I specify the date < 2014-03-01.
...
plt.plot(google_data_pre_2014['date'], 
         np.ones(len(google_data_pre_2014['date']))*
             np.mean(google_data_pre_2014['open']))
plt.xlabel('date')
...
```{code-cell} ipython3
dprice = np.diff(google_data['open'])
plt.plot(google_data['date'][1:], dprice)
plt.xlabel('date')
...
rng = default_rng(42)
N_models = 100
dprice_model = rng.normal(size = (len(google_data), N_models), loc = 0.1, scale = 16.93)
...
plt.plot(google_data['date'], price_model, alpha = 0.3);

plt.plot(google_data['date'], google_data['open'], c = 'k', label = 'NYSE data')
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```
